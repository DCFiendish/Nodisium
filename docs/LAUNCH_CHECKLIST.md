...
**World/theme — WorldPainter method being replaced (2026-09-07)**
- User found a public map to use instead of the WorldPainter/SRTM15+ pipeline. **Several chunks on
  it are corrupted — root cause not yet diagnosed.** Need: map source/format, what tool is loading
  it, and what the corruption actually looks like (crash on load? `Unknown block` errors like the
  old grass-name bug? missing chunks? garbled terrain?) before this can be fixed.
- Node/territory painting — not started on the new map. `nodes.soy` (the original phonon
  documentation site) has an editor/doc for this; use it as the reference for the painting process.
...
- Rivers/hydrology — zero data sourced yet (moot if the WorldPainter pipeline is fully dropped for
  the new public map — confirm whether the new map already has rivers).
- Custom terrain brushes per biome zone — moot for the same reason, unless the new map still needs
  biome-appropriate dressing.
...
- **Real terrain (WorldPainter/SRTM15+ pipeline)**: elevation, sea-level, biome zoning (real
  precipitation-based, not just latitude) all working; tree layers wired for 5 zones. Still open:
  custom-terrain export bug (wrong block written despite correct Java-level state), Mediterranean
  sclerophyll species (cork oak/olive — two source packs investigated and rejected), Atlantic
  moorland/heath groundcover. See `tools/agadir-mapgen/README.md`.
